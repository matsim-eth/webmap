...
class LineplotNetworkDistancePurposeProvider(DataProvider):
    """Network-distance lineplot grouped by trip purpose.
    ...
    ROUTE = "lineplot_network_distance_data_purpose.json"
        ...
        if "Microcensus" in sources:
            ...
            raw = con.execute(f"""
                SELECT p.canton_id, t.purpose,
                       t.network_distance / 1000.0 AS dist_km
                FROM read_parquet(?) t
        ...
        if "Synthetic" in sources:
            ...
            raw = con.execute(f"""
                SELECT p.canton_id, t.end_activity_type,
                       t.traveled_distance / 1000.0 AS dist_km
                FROM read_parquet(?) t
