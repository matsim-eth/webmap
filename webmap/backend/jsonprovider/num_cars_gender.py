...
class num_cars_gender(FileProvider):
    FILE = "num_cars_gender.json"
    ...
    def deliver(self, flt):
        ...
        microcensus_rows = con.execute("""
            SELECT canton_id, sex, car_availability
            FROM read_parquet(?)
            WHERE canton_id IS NOT NULL AND sex IS NOT NULL AND car_availability IS NOT NULL
        ...
        def tally(source, canton_id, sex, val):
            try:
                g = str(int(sex))
                cc = str(int(val))
            except Exception:
            ...
            counts[(source, canton_id, g, cc)] = counts.get((source, canton_id, g, cc), 0) + 1
        ...
        for canton_id, sex, val in synthetic_rows:
            tally("Synthetic", int(canton_id), sex, val)

        for canton_id, sex, val in microcensus_rows:
            tally("Microcensus", int(canton_id), sex, val)
            ...
            for source in ("Synthetic", "Microcensus"):
                for g in genders:
                    ...
                    for cc in car_classes:
                        ...
                        out.setdefault(canton_name, {}).setdefault(source, {}).setdefault(g, {})[cc] = share
