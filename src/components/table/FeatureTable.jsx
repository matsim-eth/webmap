...
const FeatureTable = ({
  ...
  const baseRows = useMemo(() => {
    ...
    // small placeholder so you see the table even with no data
    return [
      { rowKey: "p-1", tableId: 1, segmentLabel: "Segment A", directionId: "→", length: 120.5, freeSpeed: 60, capacity: 800, dailyAvg: 150, modes: "car,truck", coords: [[8.5,46.9],[8.51,46.91]] },
      { rowKey: "p-2", tableId: 2, segmentLabel: "Segment B", directionId: "←", length: 310.2, freeSpeed: 50, capacity: 600, dailyAvg: 90,  modes: "car",      coords: [[8.6,46.88],[8.62,46.89]] },
    ];
  }, [rows, geojson]);
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // Build static header/structure (your HTML pattern)
      el.innerHTML = `
        <thead>
          <tr>
            <th>Segment IDs</th>
  ...
  }, [tableRows, baseRows.length, selectedModes, useScroller, height, pageLength, showButtons, onRowClick, onSelectCoords]);
